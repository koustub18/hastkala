  ...
  const fetchArtisans = async () => {
    ...
    try {
      ...
      fetchedArtisans.sort((a, b) => {
        const dateA = a.verification?.submittedAt?.toMillis() || a.createdAt?.toMillis() || 0;
        const dateB = b.verification?.submittedAt?.toMillis() || b.createdAt?.toMillis() || 0;
        return dateB - dateA;
      });
              ...
              {expandedId === artisan.id && (
                <div className="border-t border-earth-100 p-5 bg-earth-50/50">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="bg-white p-4 rounded border border-earth-200 space-y-3">
                          ...
                          {artisan.verification?.submittedAt && (
                            <div><span className="text-xs text-earth-500 block mb-1">Applied On</span><span className="font-medium text-earth-900">{new Date(artisan.verification.submittedAt.toMillis()).toLocaleString()}</span></div>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 justify-end pt-4 border-t border-earth-200">
                    {activeTab === 'pending' && (
                      <>
                        <button onClick={() => handleReject(artisan.id)} disabled={actionLoading === artisan.id} className="px-4 py-2 border border-red-200 text-red-600 font-medium rounded hover:bg-red-50 flex items-center gap-2 disabled:opacity-50">
